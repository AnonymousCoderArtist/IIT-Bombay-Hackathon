...
<p align="center">
  <a href="README.md">English</a> · <b>हिंग्लिश</b>
</p>
...
- **🪪 Face Recognition Attendance** — camera ya photo upload se check-in. **Anti-spoofing liveness detection** (MiniFASNet) printed photo aur phone screen wali fake image reject karti hai; UniFace (SCRFD + ArcFace) detection + identity matching karta hai. Face enrollment + token based QR check-in.
- **🤖 AI Campus Assistant** — RAG chatbot jo IIT Bombay campus FAQs ka grounded answer deta hai (sources ke saath), greetings + normal chit-chat bhi. Per-user AI settings (apni Gemini/DeepSeek key laga sakte ho).
- **📷 QR Attendance Scanner** — faculty session banata hai, students QR scan karke ya token paste karke turant attendance mark karte hain.
- **🎙️ AI Lecture Notes** — audio/voice transcription + summarization se structured study notes.
- **💼 AI Placement Match** — resume/job matching match % ke saath + feedback sentiment analysis.
- **⚡ Web Push Notifications** — assignments, events, placements ke liye real-time browser alerts (VAPID).
...
match, sentiment analysis aur **face recognition attendance** (UniFace + MiniFASNet
liveness) chalti hain. Service nahi chalayi toh bhi app chalti hai (mock fallback),
bas AI/face responses mock honge.

> Repo URL: <https://github.com/AnonymousCoderArtist/IIT-Bombay-Hackathon>

```bash
# 1. Python service setup (pehli baar) — [uv](https://docs.astral.sh/uv) install karo
cd services/ai
cp .env.example .env   # AI_PROVIDER + free API key daalo (optional)
uv sync                # deps install (pyproject.toml + uv.lock se)
...
Face check-in ke liye models **repo me bundled hain** (`services/ai/models/`, ~29MB) — koi
download nahi. Service SHA-256 se verify karke use karti hai, dobara redownload nahi hota.
**Liveness by default ON hai** (printed/photo reject) — static photo se test ke liye
`FACE_LIVENESS_DISABLED=1` ke saath start karo. Poore face-testing steps neeche
["Face Attendance testing"](#face-attendance-testing-test-card) me hain.
...
Face check-in `services/ai/` (UniFace + MiniFASNet liveness) se chalti hai. Models repo me bundled
hain (`services/ai/models/`) — **kisi bhi naye setup pe redownload/train nahi karna padta**, service
SHA-256 se verify karke directly use karti hai. Local test flow:
